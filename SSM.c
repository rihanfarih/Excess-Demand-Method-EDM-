...
int main()
{
	int i, j, jumlah_supply, jumlah_demand, flag, p, q, 
	total_supply=0, 
	total_demand=0;
	char dummy;
	FILE *in;
	...
	while(!feof(in))
	{
		...
		// Mengeset alokasi semua cell menjadi -1
		for(i=0;i<jumlah_supply;i++)
		{
			for(j=0;j<jumlah_demand;j++)
			{
		...
		for(p=0;p<jumlah_supply;p++)
		{
			for(q=0;q<jumlah_demand;q++)
			{
				if(allocation[p][q]==-1) 
					printf("%d\t",cost[p][q]);
				else printf("%d\t",allocation[p][q]);
			}
			printf("| %d\n",supply[p]);
		}
		
		for(q=0;q<jumlah_demand;q++)	printf("_______\t");
		printf("\n");
		for(q=0;q<jumlah_demand;q++)	printf("%d\t",demand[q]);
		printf("\n--------------------------------------------\n\n");
		...
		}	// Loop di atas berfungsi untuk mereserve tempat untuk nilai alokasi pada kost mendatang	
		...
		while(1)
		{
			...
			else if (jumlah_ER>0)								
			{
				...
				if(supply[baris_flc] == supply[baris_slc])	//Jika ada supply yg sama
				{
					printf("Sama");
				...
				{
					if(supply[baris_flc] > supply[baris_slc])	
					{
		...
		// Must create a function to count the total cost
		//total_alloc=0;
		for(p=0; p<jumlah_supply; p++)
		{
			for(q=0; q<jumlah_demand; q++)
		...
		printf("\n Total Alokasi tabel adalah: %d", total_alloc);
	}
}
